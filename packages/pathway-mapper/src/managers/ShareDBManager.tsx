...
export default class ShareDBManager {
  ...
  deleteShareDBObject(mapName, objectKey) {
    this.doc.submitOp(
      [{ p: [mapName, objectKey], od: this.doc.data[mapName][objectKey] }],
      this.shareDBError
    );
  }
  ...
  incrementMessageCount() {
    this.doc.submitOp(
      [{ p: [this.CHAT_MESSAGES_COUNT], na: 1 }],
      this.shareDBError
    );
  }
  ...
  addNewMessage(object: ChatMessageMetaData, chatMessageKey: number) {
    this.doc.submitOp(
      [{ p: [this.CHAT_APPLICATION_NAME, chatMessageKey], oi: object }],
      this.shareDBError
    );
  }
  ...
  incrementShareDBGroupCount() {
    this.doc.submitOp(
      [{ p: [this.GENOMIC_DATA_GROUP_COUNT], na: 1 }],
      this.shareDBError
    );
  }
  ...
  isShareDBReplaceEvent(op) {
    return (
      (op.hasOwnProperty("oi") && op.hasOwnProperty("od")) ||
  ...
  syncInitialSharedDocData() {
    ...
    if (self.doc.data[this.GENOMIC_DATA_COLOR_SCHEME_NAME]) {
      genomicDataColorScheme =
        self.doc.data[this.GENOMIC_DATA_COLOR_SCHEME_NAME][0];
    }
    ...
    for (const key of Object.keys(edgeMap)) {
      ...
      if (
        ...
        const pubmedIDs =
          tmpEdge.pubmedIDs == undefined ? [] : tmpEdge.pubmedID;
        ...
        const edgeCurveStyle = tmpEdge.edgeCurveStyle
          ? tmpEdge.edgeCurveStyle
          : "bezier";
    ...
    this.editor.highlightElementsInitially(
    ...
    for (const key_g of Object.keys(groupedGenomicDataMap)) {
      ...
      if (data.length !== 1) {
        console.log(
          "Grouped genomic data expected to be of length 1 (from sync)"
        );
      }
      data.forEach((profileId: any) => {
    ...
    this.doc.on("op", function(op, source) {
      for (var i = 0; i < op.length; i++) {
        ...
        if (path === self.CHAT_APPLICATION_NAME) {
          self.editor.updateMessages(
            self.doc.data[self.CHAT_APPLICATION_NAME][handleOp.p[1]]
          );
        }
        if (path === self.CHAT_MESSAGES_COUNT) {
          self.editor.updateMessageCount(
            self.doc.data[self.CHAT_MESSAGES_COUNT]
          );
        }

        if (path === self.PATHWAY_TITLE) {
          self.editor.updatePathwayTitleBack(
            self.doc.data[self.PATHWAY_TITLE][0]
          );
        }
  ...
  groupGenomicData(cancerNames, inGroupId) {
    ...
    if (genomicGroupMap.hasOwnProperty(groupID)) {
      this.updateShareDBObject(
        this.GENOMIC_DATA_GROUP_NAME,
        groupID,
        currentGroup
      );
    } else {
      //Insert new group
      this.insertShareDBObject(
        this.GENOMIC_DATA_GROUP_NAME,
        groupID,
        currentGroup
      );
    ...
    for (var geneSymbol in geneData) {
      ...
      for (var cancerType in geneData[geneSymbol]) {
        if (!(cancerType in genomicMapEntry))
          genomicMapEntry[cancerType] = parseInt(
            geneData[geneSymbol][cancerType]
          ).toFixed(2);
      }
      ops.push({
  ...
  loadGraph(nodes, edges) {
    ...
    for (const edge of edges) {
      ...
      if (
        ...
        edge.data.bendPointPositions.length > 0
      ) {
        ...
        edge.data.controlPointPositions.length > 0
      ) {
  ...
  layoutPropertiesInitializer(params) {
    ...
    layoutProperties.gravityRangeCompound =
      params.gravityRangeCompound || "undefined";
    ...
    layoutProperties.tilingPaddingVertical =
      params.tilingPaddingVertical || "undefined";
    layoutProperties.tilingPaddingHorizontal =
      params.tilingPaddingHorizontal || "undefined";
    layoutProperties.initialEnergyOnIncremental =
      params.initialEnergyOnIncremental || "undefined";
